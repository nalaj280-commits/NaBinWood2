...
// ============================================================
// [수정] 한글 너비를 고려한 고정폭 출력 헬퍼
// total_cols: 채워야 할 화면 칸 수
// ============================================================
void print_fixed_width(const wchar_t* text, int total_cols)
{
    int w = get_width(text);
    // 텍스트가 너무 길면 잘라내기 (안전 처리)
    if (w >= total_cols) {
        print_buf(L"%ls", text);
        return;
    }
    print_buf(L"%ls", text);
    int pad = total_cols - w;
    for (int i = 0; i < pad; i++)
        print_buf(L" ");
}
...

{
    ...
    if (col >= room_limit_width[room] || row >= room_limit_height[room]) {
        ...
        set_color_buf(COLOR_DARKGRAY);  print_buf(L"┼ "); set_color_buf(COLOR_WHITE);

    }

    else if (room == 0 && !isFinalKeyPicked && finalKeyX != -10 && row == finalKeyY && col == finalKeyX) {

        set_color_buf(COLOR_RED);  print_buf(L"⚷ "); set_color_buf(COLOR_WHITE);
        ...
        set_color_buf(COLOR_BLUE); print_buf(L"♒ "); set_color_buf(COLOR_WHITE);
...

{
    ...
    // ── 2번째 줄: 방 이름 | 상태 ─────────────────────────────
    // 구조: "| " + [방이름 26칸] + " | " + [상태 31칸] + " |"
    // 총 너비: 2 + 26 + 3 + 31 + 2 = 64 ... 실제 박스는 81자
    // 원본 박스 너비에 맞게 방이름=26칸, 상태=31칸으로 고정
    move_cursor_buf(0, 1);
    print_buf(L"| ");
    set_color_buf(COLOR_CYAN);
    print_fixed_width(roomName, 26);
    set_color_buf(COLOR_WHITE);
    print_buf(L" | ");
    set_color_buf(isHidden ? COLOR_GREEN : COLOR_RED);
    print_fixed_width(isHidden ? L"옷장에 숨음 (SAFE)" : L"추격당하는 중...", 31);
    set_color_buf(COLOR_WHITE);
    print_buf(L" |");
            ...
            if (!spacePressed) {
                ...
                if (currentRoom == 0 && targetDoorTile == 4) {
                    ...
                    while (1) {
                        ...
                        if (ch >= '1' && ch <= '3') { answer = ch - '0'; break; }
